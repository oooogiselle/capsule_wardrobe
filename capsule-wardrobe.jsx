...
const css = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Layout ── */
  .page { padding: 36px 40px; max-width: 1200px; margin: 0 auto; }
...
async function saveData(items) {
  try { await window.storage.set(STORAGE_KEY, JSON.stringify(items)); } catch {}
}
...
async function saveGoals(goals) {
  try { await window.storage.set(GOALS_KEY, JSON.stringify(goals)); } catch {}
}
...
async function saveBudget(b) {
  try { await window.storage.set(BUDGET_KEY, JSON.stringify(b)); } catch {}
}
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
function DashboardTab({ items, goals, totalItems, totalSpent, totalWears, avgCpw, adjustWorn, openEdit }) {
